...
def generador_grafo_no_conexo(numero_ciudades,porcentaje_premium):
    ...
    res = caminos
    cantidad_a_borrar = random.sample(range(1,len(res) + 1), 1)[0]
    indice_elementos_a_borrar = random.sample(range(0,len(res)), random.randrange(1,len(res) + 1))
...
if __name__ == "__main__":
    ...
    while(ciudades <= 100):
        for i in range(0,100):
            generador_grafo_no_conexo(ciudades,porcentaje_premium)
        ciudades+=1
    print(str(-1) + " " + str(-1) )
