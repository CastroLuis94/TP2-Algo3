...
vector< vector <int> > Representador::matrizConPremiums(int k){
    ...
    //vector<vector<int> >  res((N+1)*(k+1), vector<int>((N+1)*(k+1), -1));   
    int i = 0;
   // vector< vector<int> >& vs = _listaEntrada;
    while(i < _listaEntrada.size()){     //    
         int ciudad1 = _listaEntrada[i][0]*(k+1);
         int ciudad2 =  _listaEntrada[i][1]*(k+1);
         int ciudadaux;         
         int distancia = _listaEntrada[i][3];
         bool esPremium = _listaEntrada[i][2];
         if(esPremium){
            ciudadaux= ciudad2 +1;
         }else{
            ciudadaux = ciudad2;
         }
        for(int j = ciudad1; j< ciudad1+(k+1); j++){ // k+1                
        ...
        for(int j = ciudad2; j< ciudad2+(k+1); j++){ //k+1                
