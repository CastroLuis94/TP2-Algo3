    ...
    // antecesor[];   
     ...
     for(int count = (k+1); count < totalarreglo; count++)//decia N-1 totalarreglo-1
     {
       ...
       for (int n = (k+1)+ (u % (k+1)); n < totalarreglo; n= n + (k+1)){  
            //cout<<" Esto estaba bien???" << matriz[1][2]<< endl;
            //cout << "n "<< n <<" u "<< u<<"S[n] "<<S[n]<< " distancias[n]"<< distancias[n] <<" distancias[u]+ matriz" <<distancias[u]+(matriz[u][n]) <<endl;
            if (!S[n] && ((matriz[u][n]) > -1)  && distancias[u]+(matriz[u][n]) < distancias[n]){//&& (distancias[u]!= INT_MAX)
             distancias[n] = distancias[u] + (matriz[u][n]); 
             //cout << "distancia[n]"<< distancias[n]<<endl;                        
            }
        }
...
int main(){
    while(true){
       ...
       // mostrar(matriz);
        ...
        //cout << distancias[4]<<distancias[5]<< endl;
